...
<div class="mb-6 flex  items-center gap-x-4">
    <h1>کاڵاکان</h1>
    <a href="{{ route('supplier.create') }}" class="bg-[#6A64F1] w-32 text-center p-2 text-white rounded-sm font-bold">زیدکردنی کۆمپانیا</a>
        ...
        <tbody>
            @foreach ($data as $row)
                <tr class="bg-gray-100 text-center border-b  text-gray-600">
                    ...
                    <td class="p-2 border-r">{{ $row->phone_number }}</td>
                    <td class="p-2 border-r">{{ $row->address }}</td>
                    <td class="border-r">
                        <a href="{{ route('supplier.edit',$row->id) }}" class="bg-blue-500 p-2 text-white hover:shadow-lg text-xs font-thin">Edit</a>
                        <form action="{{ route('supplier.destroy',[$row->id]) }}" method="post" class="inline">
                            @csrf
                            @method('DELETE')
